...
const Table: React.FC<TableProps> = ({ tableColumns, api }) => {
  ...
  // Raw Data
  const [rawData, setRawData] = useState<FlightData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response: FlightData[] = res.data.data.content;
        console.log(res.data.data.content);
        // const response: FlightData[] = [];
        // const response: FlightData[] = tableData;
            ...
            <tbody>
              {data.map((item) => (
                <tr key={item.id} className="border-b-2">
                  ...
                  <td className="px-4 py-3 text-center">
                    {item.businessSeatsPrice}
                  </td>
                  <td className="px-4 py-3 text-center">
                    {item.firstSeatsPrice}
                  </td>
