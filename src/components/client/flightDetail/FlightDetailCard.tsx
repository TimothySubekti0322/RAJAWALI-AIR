...
interface FlightDetailCardProp {
    airplaneCode: string
}
const FlightDetailCard = ({airplaneCode}: FlightDetailCardProp) => {
    const classTypeLs = localStorage.getItem("classType") as string;

    return(
        <div className={'container flex flex-col my-4 border border-1 border-slate-400 rounded-md p-3 pb-0'}>
            <div className={'flex gap-3 rounded-md'}>
                ...
                <div>
                    ...
                    <p className={'text-slate-500 text-xs font-medium'}>{airplaneCode} | {classTypeLs.replace(/"/g, '')}</p>
