...
import ContactDetailForm, {ContactDetailInput} from "../../components/client/fillDetailInformation/ContactDetailForm";
    ...
    const passengerDetails = () => {
        ...
        if (dataPassenger.adultTotal > 0) {
            for (let i = 0; i < dataPassenger.adultTotal; i ++) {
                passengers.push(<SwipeableEdgeDrawer index={index} ageGroup={"Adult"} />);
                index ++;
            }
        }
        if (dataPassenger.childTotal > 0) {
            for (let i = 0; i < dataPassenger.childTotal; i ++) {
                passengers.push(<SwipeableEdgeDrawer index={index} ageGroup={"Child"} />);
                index ++;
            }
        }
        if (dataPassenger.infantTotal > 0) {
            for (let i = 0; i < dataPassenger.infantTotal; i ++) {
                passengers.push(<SwipeableEdgeDrawer index={index} ageGroup={"Infant"} />);
                index ++;
            }
        }
        ...
        <section className={'className="w-full min-h-screen bg-[#f7f7f7] relative"'}>
            ...
            <BodyLayout paddingBottomSize="1rem">
                <div className={'px-4 pt-6 pb-0'}>
                    <FlightCard id={id ?? "1"} />
                    <ContactDetailForm contactDetail={contactDetail} setContactDetail={setContactDetail} />
                    <div>
                        <p className={'text-sm font-bold text-black mb-2'}>Passenger Details</p>
                        {passengerDetails()}
                    </div>
                    <button onClick={onNext} className="btn btn-block bg-[#1E90FF] text-white hover:bg-blue-700">Next</button>
                </div>
            </BodyLayout>
